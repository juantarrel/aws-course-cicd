...
#CLOUD FORMATION
aws cloudformation create-stack --stack-name elb-python-prod-$BUILD_NUMBER --template-body https://public-aws-course-cicd-script.s3.amazonaws.com/node_prod_2019_07_14.json --parameters '
[
   {
      "ParameterKey": "AZs",
...
build-docker() {
    cp web_docker/* .
    aws ecr get-login --no-include-email --region=us-east-1 | sh
    docker build --build-arg APP_DOMAIN=$1 -t python-base .
    docker tag node-base:latest 207155759131.dkr.ecr.us-east-1.amazonaws.com/python-prod-app:$BUILD_NUMBER
    docker push 207155759131.dkr.ecr.us-east-1.amazonaws.com/python-prod-app:$BUILD_NUMBER
    rm Dockerfile
